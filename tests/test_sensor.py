...
async def test_sensor_setup(hass):
    ...
    hass.data[DOMAIN][config_entry.entry_id] = type(
        "MockCoordinator", (), {"data": mock_data}
    )()
    ...
    assert (
        humidity_sensor.native_unit_of_measurement == "%"
    )  # Should be converted from %RH
    ...
    assert (
        timestamp_sensor.native_unit_of_measurement == ""
    )  # Empty string for timestamp sensors
...
def test_device_class_guessing():
    ...
    temp_sensor = EasylogCloudSensor(
        mock_coordinator, mock_device, "Temperature", {"value": 25.5}
    )
    assert temp_sensor._attr_device_class == SensorDeviceClass.TEMPERATURE

    # Test humidity detection
    humidity_sensor = EasylogCloudSensor(
        mock_coordinator, mock_device, "Humidity", {"value": 60}
    )
    assert humidity_sensor._attr_device_class == SensorDeviceClass.HUMIDITY

    # Test CO2 detection
    co2_sensor = EasylogCloudSensor(
        mock_coordinator, mock_device, "CO2 Level", {"value": 400}
    )
    assert co2_sensor._attr_device_class == SensorDeviceClass.CO2

    # Test pressure detection
    pressure_sensor = EasylogCloudSensor(
        mock_coordinator, mock_device, "Pressure", {"value": 1013}
    )
    assert pressure_sensor._attr_device_class == SensorDeviceClass.PRESSURE

    # Test signal strength detection
    signal_sensor = EasylogCloudSensor(
        mock_coordinator, mock_device, "Signal Strength", {"value": -50}
    )
    assert signal_sensor._attr_device_class == SensorDeviceClass.SIGNAL_STRENGTH

    # Test timestamp detection
    timestamp_sensor = EasylogCloudSensor(
        mock_coordinator, mock_device, "Last Updated", {"value": "2024-01-01"}
    )
    assert timestamp_sensor._attr_device_class == SensorDeviceClass.TIMESTAMP

    # Test unknown sensor
    unknown_sensor = EasylogCloudSensor(
        mock_coordinator, mock_device, "Unknown", {"value": 100}
    )
...
def test_state_class_guessing():
    ...
    voc_sensor = EasylogCloudSensor(
        mock_coordinator, mock_device, "VOC", {"value": 150}
    )
    assert voc_sensor._attr_state_class == "measurement"

    particulate_sensor = EasylogCloudSensor(
        mock_coordinator, mock_device, "PM2.5", {"value": 10}
    )
    assert particulate_sensor._attr_state_class == "measurement"

    aqi_sensor = EasylogCloudSensor(
        mock_coordinator, mock_device, "Air Quality", {"value": 50}
    )
    ...
    regular_sensor = EasylogCloudSensor(
        mock_coordinator, mock_device, "Temperature", {"value": 25.5}
    )
...
def test_numeric_sensor_detection():
    ...
    aqi_sensor = EasylogCloudSensor(
        mock_coordinator, mock_device, "Air Quality Index", {"value": 50}
    )
...
def test_sensor_native_value_property():
    ...
    mock_coordinator = type("MockCoordinator", (), {"data": mock_data})()
    ...
    timestamp_sensor = EasylogCloudSensor(
        mock_coordinator, mock_device, "Last Updated", {"value": "2024-01-01T12:00:00"}
    )
    ...
    invalid_timestamp_sensor = EasylogCloudSensor(
        mock_coordinator, mock_device, "Invalid Timestamp", {"value": "invalid"}
    )
    ...
    invalid_numeric_sensor = EasylogCloudSensor(
        mock_coordinator, mock_device, "Invalid Numeric", {"value": "not a number"}
    )
    assert (
        invalid_numeric_sensor.native_value == "not a number"
    )  # Returns raw value for non-numeric sensors

    # Test sensor with device not found in coordinator
    non_existent_sensor = EasylogCloudSensor(
        mock_coordinator,
    ...
    missing_label_sensor = EasylogCloudSensor(
        mock_coordinator, mock_device, "Missing Label", {"value": 25.5}
    )
...
def test_humidity_unit_conversion():
    ...
    humidity_sensor_rh = EasylogCloudSensor(
        mock_coordinator, mock_device, "Humidity", {"value": 60, "unit": "%RH"}
    )
    assert humidity_sensor_rh.native_unit_of_measurement == "%"

    # Test RH% unit conversion
    humidity_sensor_rh2 = EasylogCloudSensor(
        mock_coordinator, mock_device, "Humidity", {"value": 60, "unit": "RH%"}
    )
    assert humidity_sensor_rh2.native_unit_of_measurement == "%"

    # Test regular % unit (no conversion needed)
    humidity_sensor_percent = EasylogCloudSensor(
        mock_coordinator, mock_device, "Humidity", {"value": 60, "unit": "%"}
    )
...
def test_diagnostic_entity_categories():
    ...
    firmware_sensor = EasylogCloudSensor(
        mock_coordinator, mock_device, "Firmware Version", {"value": "1.2.3"}
    )
    assert firmware_sensor.entity_category == EntityCategory.DIAGNOSTIC

    mac_sensor = EasylogCloudSensor(
        mock_coordinator, mock_device, "MAC Address", {"value": "AA:BB:CC:DD:EE:FF"}
    )
    assert mac_sensor.entity_category == EntityCategory.DIAGNOSTIC

    ssid_sensor = EasylogCloudSensor(
        mock_coordinator, mock_device, "SSID", {"value": "MyWiFi"}
    )
    assert ssid_sensor.entity_category == EntityCategory.DIAGNOSTIC

    wifi_signal_sensor = EasylogCloudSensor(
        mock_coordinator, mock_device, "Wi-Fi Signal", {"value": -50}
    )
    assert wifi_signal_sensor.entity_category == EntityCategory.DIAGNOSTIC

    wifi_signal_sensor2 = EasylogCloudSensor(
        mock_coordinator, mock_device, "WiFi Signal", {"value": -50}
    )
    ...
    assert (
        not hasattr(temp_sensor, "entity_category")
        or temp_sensor.entity_category is None
    )
...
def test_sensor_native_value_timestamp_parsing():
    ...
    timestamp_sensor2 = EasylogCloudSensor(
        mock_coordinator, mock_device, "Last Updated", {"value": "2024-01-01T12:00:00"}
    )
...
def test_sensor_native_value_numeric_sensor():
    ...
    voc_sensor = EasylogCloudSensor(
        mock_coordinator, mock_device, "VOC", {"value": "150.5"}
    )
...
async def test_sensor_native_value_timestamp_string_parsing_failure(hass):
    ...
    sensor = EasylogCloudSensor(
        ...
        coordinator.data[0],
        "Last Updated",
        {"value": "invalid timestamp string", "unit": ""},
    )
...
async def test_sensor_native_value_numeric_sensor_float_conversion(hass):
    ...
    sensor = EasylogCloudSensor(
        coordinator, coordinator.data[0], "VOC", {"value": "25.5", "unit": "ppm"}
    )
...
async def test_sensor_native_value_numeric_sensor_conversion_failure(hass):
    ...
    sensor = EasylogCloudSensor(
        ...
        coordinator.data[0],
        "PM2.5",
        {"value": "invalid number", "unit": "µg/m³"},
    )
...
def test_sensor_native_value_timestamp_parsing_failure_specific():
            ...
            "data": [
                {
                    ...
                    "model": "EL-USB-TC",
                    "Last Updated": {
                        "value": "completely invalid timestamp",
                        "unit": "",
                    },
                }
            ]
    ...
    sensor = EasylogCloudSensor(
        ...
        "Last Updated",
        {"value": "completely invalid timestamp", "unit": ""},
    )
    sensor._attr_device_class = SensorDeviceClass.TIMESTAMP
...
def test_sensor_native_value_numeric_sensor_conversion_failure_specific():
    ...
    sensor = EasylogCloudSensor(
        ...
        mock_coordinator.data[0],
        "PM2.5",
        {"value": "not a number at all", "unit": "µg/m³"},
    )
...
def test_sensor_native_value_timestamp_parsing_failure_line_104():
    """Test sensor native_value with timestamp parsing failure specifically for line 104."""
    from homeassistant.components.sensor import SensorDeviceClass
            ...
            "data": [
                {
                    ...
                    "name": "Test Device",
                    "model": "EL-USB-TC",
                    "Last Updated": {"value": "invalid timestamp format", "unit": ""},
                }
            ]
        },
    )()
    ...
    sensor = EasylogCloudSensor(
        ...
        mock_coordinator.data[0],
        "Last Updated",
        {"value": "invalid timestamp format", "unit": ""},
    )
    sensor._attr_device_class = SensorDeviceClass.TIMESTAMP
...
def test_sensor_native_value_numeric_sensor_conversion_failure_lines_115_116():
            ...
            "data": [
                {
                    ...
                    "name": "Test Device",
                    "model": "EL-USB-TC",
                    "PM2.5": {"value": "definitely not a number", "unit": "µg/m³"},
                }
            ]
        },
    )()
    ...
    sensor = EasylogCloudSensor(
        ...
        mock_coordinator.data[0],
        "PM2.5",
        {"value": "definitely not a number", "unit": "µg/m³"},
    )
